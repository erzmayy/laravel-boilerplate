    ...
    <div class="row">
        <table class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0"
            ...
            <tr>
                ...
                <th>@sortablelink('diskon', __('views.admin.purchase.order.lines.index.table_header_3'),['page' => $purchaseOrderLines->currentPage()])</th>
                <th>@sortablelink('total', __('views.admin.purchase.order.lines.index.table_header_2'),['page' => $purchaseOrderLines->currentPage()])</th>
                <th>@sortablelink('created_at', __('views.admin.purchase.order.lines.index.table_header_4'),['page' => $purchaseOrderLines->currentPage()])</th>
                <th>@sortablelink('updated_at', __('views.admin.purchase.order.lines.index.table_header_5'),['page' => $purchaseOrderLines->currentPage()])</th>
                <th>Actions</th>
            </tr>
            </thead>
            <tbody>
            @foreach($purchaseOrderLines as $purchaseOrderLine)
                <tr>
                    ...
                    <td>{{ $purchaseOrderLine-> diskon }}</td>  
                    <td>{{ $purchaseOrderLine->total }}</td>
                    <td>{{ $purchaseOrderLine->created_at }}</td>
                    <td>{{ $purchaseOrderLine->update_at }}</td>
                    <td>
